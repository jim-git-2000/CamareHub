...
## 调试 5：美化 Dashboard / Stats 统计卡片与最近新增器材卡片
...
- 只调整四个统计卡片的视觉样式。
- 只调整 dashboard 中“最近新增器材”的卡片视觉样式。
- 可以抽取共享样式配置。
- 不改变统计数据来源和计算逻辑。
- 不改变“最近新增器材”的展示内容。
...
- 内容保持当前信息，不新增字段，不删除字段。
- 每条器材记录做成独立卡片。
- 单条卡片尺寸对标 dashboard 的“最近拍摄事项”卡片。
- 背景使用低饱和度纯色。
- 可以按 item 类型使用不同低饱和度背景色，例如：
...
- 数字、标题、辅助说明不溢出。
- 页面没有大面积单一色系压迫感。
- dashboard 的“最近新增器材”每条记录变成独立美化卡片。
- 最近新增器材内容不变。
- 最近新增器材卡片大小对标最近拍摄事项卡片。
- 最近新增器材卡片背景为低饱和度纯色。
- 最近新增器材卡片文本、badge、估值信息可读性正常。
- 前端构建通过。
...
4. 检查 dashboard 最近新增器材卡片是否与最近拍摄事项尺寸协调。
5. 检查最近新增器材内容是否保持不变。
```
...
1. 将导航栏改为简洁中文。
2. Stats 页面镜头焦段统计改为两种分类方式：
   - 定焦 / 变焦
   - 超广角 / 广角 / 中焦 / 中长焦 / 长焦 / 超长焦
...
- 修改导航栏显示文案，不改路由。
- 修改 Stats 相关后端统计接口、前端 API helper、前端 Stats 页面。
- 保留现有统计接口兼容性，除非明确无调用方。
- 不改变器材录入表单字段。
- 不改变已有 item、lens 数据结构。
- 不执行其他调试任务。
...
- `focal_length_min` 和 `focal_length_max` 都存在
- 且二者不同

缺少焦距：

- 不计入定焦 / 变焦统计。
- 后续如需要可单独增加“未知焦段”桶，本次不做。

---

### 超广角 / 广角 / 中焦 / 中长焦 / 长焦 / 超长焦分类规则
...
```text
17-40mm：计入 超广角、广角、中焦
15-45mm：计入 超广角、广角、中焦
40mm：计入 中焦
50mm：计入 中焦
70-200mm：计入 中长焦、长焦
300mm：计入 超长焦
...
```text
GET /api/stats/lens-zoom-type
GET /api/stats/lens-focal-category
```
...
- 导航显示为 `概览 / 器材 / 照片 / 统计 / 设置`。
- 导航 active 状态仍正常。
- 移动端导航横向滚动仍正常。
- `/stats` 页面可以正常加载。
- Stats 页面出现“定焦 / 变焦”图表。
- Stats 页面出现“焦段分类”图表。
- `17-40mm` 这类变焦镜头可以同时计入超广角、广角和中焦。
- `15-45mm` 这类变焦镜头可以同时计入超广角、广角和中焦。
- `40mm`、`50mm` 这类定焦镜头计入中焦。
- `70-200mm` 这类变焦镜头可以同时计入中长焦和长焦。
- `300mm` 及以上镜头计入超长焦。
- 前端构建通过。
- 后端语法检查通过。
...
1. 打开任意页面，确认导航显示中文。
2. 打开 /stats。
3. 确认“定焦 / 变焦”和“焦段分类”图表显示。
4. 对照当前镜头数据检查计数是否符合规则。
```
