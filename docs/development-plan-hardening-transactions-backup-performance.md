...
- 制定日期：2026-08-13
- 适用版本基线：`0.3.0`
- 适用范围：前端发布门禁、SQLite 兼容迁移、器材交易界面、备份恢复、器材与拍摄事项列表性能及筛选导航状态
...
2026-08-13 实施进度：阶段 0–5 已完成；器材列表封面、facets、器材与拍摄事项渐进加载、超过 100 条数据的完整读取、拍摄事项器材选择器服务端搜索，以及器材筛选 URL 与详情返回状态恢复均已落地。后端 23 项回归测试、前端 lint、TypeScript 与 production build 已通过。当前环境没有 Docker，尚不能执行 `docker compose config` 和容器内维护命令验收，因此本计划继续保留。
...
- 器材列表的关键词、类型、品牌、状态、卡口、相机类型和已加载页数以 URL 查询参数为唯一可恢复状态；只在 URL 中写入非默认值，使筛选结果可刷新、收藏和分享。
- 离散筛选变化立即更新 URL；关键词输入使用短延迟合并更新，避免每次按键制造浏览器历史。任何筛选变化都把已加载页数重置为第一页，“加载更多”同步增加 URL 中的页数。
- 从筛选结果进入器材详情时，详情页的显式“返回器材”动作携带并校验当前列表返回地址；直接访问详情或返回参数无效时退回 `/items`，不得接受站外地址。
- 返回器材列表时先按 URL 恢复筛选条件并加载到记录的页数，再交由浏览器恢复滚动位置，避免出现筛选控件已恢复但目标卡片尚未加载的状态。
...
7. 将器材筛选和已加载页数改为 URL 查询参数驱动，统一参数解析、默认值省略、非法值降级和筛选变化时的第一页重置。
8. 器材卡片进入详情时传递经过编码的站内返回地址；详情页返回动作优先回到该地址，浏览器后退仍保持原生历史行为。
9. 从详情返回时按 URL 重新请求已记录的所有分页，确认筛选控件、结果数量、已加载范围和滚动恢复顺序一致。
...
- 分页前后排序稳定，无重复、无漏项。
- 现有 API 旧字段和默认排序保持兼容。
- 筛选后地址栏能表达完整列表状态；刷新或复制该链接可得到相同筛选结果和已加载范围。
- 从筛选列表进入器材详情后，通过浏览器后退或详情页“返回器材”都能恢复进入前的筛选状态；直接进入详情时返回 `/items`。
- 连续输入关键词不会为每个字符产生一条浏览器历史，非法或站外返回参数不会造成错误导航或开放重定向。
...
- [x] 器材筛选和已加载页数由 URL 查询参数表达，刷新及分享链接可复现列表状态。
- [x] 从器材详情返回时恢复进入前的筛选条件、已加载范围和滚动位置，非法返回地址安全降级。
