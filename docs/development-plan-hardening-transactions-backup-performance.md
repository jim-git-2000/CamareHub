...
- [ ] 前端 lint 为 0 error、0 warning。
- [ ] 前端 production build 成功。
- [ ] 后端语法编译与全部回归测试通过。
- [ ] 旧数据库升级前自动创建可验证备份。
- [ ] 重复图片记录的原始字段完整保存在冲突归档表和迁移前备份中。
- [ ] 迁移重复启动不产生重复归档、重复删除或索引错误。
- [ ] 数据库迁移可按文档回滚，并可再次升级。
- [ ] 迁移前后 `uploads/` 文件数量和 SHA-256 不变。
- [ ] 器材详情页支持交易新增、查看、编辑和删除。
- [ ] 交易 CRUD 不隐式修改器材状态、价格或估值。
- [ ] 一条命令可生成包含数据库、一言配置、上传文件和 manifest 的备份。
- [ ] 损坏、篡改或路径不安全的归档会在覆盖数据前被拒绝。
- [ ] 恢复前自动备份；恢复失败可自动回到恢复前状态。
- [ ] 器材列表没有 N+1 图片请求。
- [ ] 第 101 条之后的器材和拍摄事项可被查看、筛选和选择。
- [ ] API、数据库、README、环境变量示例与最终实现一致。
- [ ] 用户数据目录和备份目录没有新增 Git 跟踪文件。
